...
用法:
  python3 scripts/telemetry-grabber.py --key <BASE64密钥>
  python3 scripts/telemetry-grabber.py --key <BASE64密钥> 0.0.0.0 9000

依赖: pip install cryptography

--key 为部署特定的遥测密钥:Base64 编码的 32 字节随机值(openssl rand -base64 32),
必须与服务端 telemetry.key 文件 / 客户端 config.conf 的 telemetry.key 一致。
...
报文格式(Java 端 TelemetryManager):
  ...
  [18..]   AES-256-GCM 密文(JSON + 16 字节认证标签)
前 6 个头部字节(magic+version+type)作为 GCM AAD 绑定认证
"""
...
def decrypt(data, key):
    """校验信封并返回解密后的 JSON 文本;头部 6 字节作为 GCM AAD"""
    ...
    plaintext = AESGCM(key).decrypt(iv, ciphertext, data[:6])
    return TYPE_NAMES.get(msg_type, f"type-{msg_type}"), plaintext.decode("utf-8")


def listen(host, port, key):
...
def main():
    ...
    parser.add_argument("--key", required=True,
                        help="遥测密钥:Base64 编码的 32 字节(与服务端 telemetry.key 一致)")
    ...
    try:
        ...
        print(f"[!] 密钥不是合法的 Base64: {e}")
        sys.exit(1)
    if len(key) != 32:
        print(f"[!] 密钥解码后应为 32 字节(AES-256),实际 {len(key)} 字节")
        sys.exit(1)
    try:
        listen(args.host, args.port, key)
