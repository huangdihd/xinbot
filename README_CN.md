...
"telemetry" : {
    ...
    "key" : ""            // 部署密钥：Base64 编码的 32 字节随机数，
                           // 例如 `openssl rand -base64 32`，须与服务端一致
    // 字段级隐私开关：sendBot、sendServer、sendState、
    // sendPlayers、sendUptime、sendSystem（默认均开启）
}
```

**加密** —— 每个数据包都是 "XBTL" 信封（magic + 版本 + 类型 + IV + 密文），使用上述部署密钥以 AES-256-GCM 加密，密钥必须与你的服务器一致。6 字节头部作为 GCM AAD 与密文绑定，且 JSON 载荷的 `type` 必须与信封类型一致，因此篡改或使用错误密钥的数据包会被拒收；未配置密钥时不会发送任何数据（fail-closed）。UDP 与 HTTP 两种传输使用相同的信封格式。
